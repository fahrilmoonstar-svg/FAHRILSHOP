...
app = Flask(__name__)
app.secret_key = "fahrilshop_aman_2026"

# === DATA LANGSUNG DI KODE ===
# Data ini akan terbaca, dan bisa diubah lewat panel admin
data_toko = {
    "produk": [
    ...
    if request.method == "POST":
        try:
            ...
            if barang:
                ...
                return render_template("index.html", produk=data_toko["produk"], hasil=pesanan)
        except:
            pass
    return render_template("index.html", produk=data_toko["produk"])
...
def kelola():
    ...
    if request.method == "POST":
        try:
            ...
            if aksi == "tambah":
                baru = {
                    "id": max(p["id"] for p in data_toko["produk"]) + 1 if data_toko["produk"] else 1,
...
def lap():
    ...
    total = sum(p["total"] for p in daftar) if daftar else 0
