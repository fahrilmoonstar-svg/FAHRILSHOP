...
# PENGATURAN JSONBIN
# --------------------------
JSONBIN_API_KEY = "$2a$10$EcY7eevXYKOMgUsuNSKjW.mKxnU2FcMpnAFPt5pAfQntV27ugB/Am"  # Pakai Master Key
...
def baca_data():
    try:
        res = requests.get(f"https://api.jsonbin.io/v3/b/{JSONBIN_ID}", headers=HEADERS, timeout=10)
        res.raise_for_status()
        return res.json()["record"]
    except Exception as e:
        print("Gagal baca data:", str(e))
        # Data cadangan kalau koneksi bermasalah
        return {
            "produk": [
...
def simpan_data(data):
    try:
        requests.put(f"https://api.jsonbin.io/v3/b/{JSONBIN_ID}", json=data, headers=HEADERS, timeout=10)
    except Exception as e:
        print("Gagal simpan data:", str(e))
...
def buat_laporan(periode="semua"):
    ...
    total = sum(p["total"] for p in laporan) if laporan else 0
